...
void snmpAgent::addUser(const char* userName, authType auth, privType priv, const char* authPass, const char* privPass)
{
	...
	if (auth != 0)
	{
		if (authPass != NULL)
		{
			if (auth == SHA)
			{
				password_to_key_sha((unsigned char*)authPass, (int)strlen(authPass), this->engineID, this->engineIDLength, (unsigned char*)this->users[this->numberOfUsers].authKey);
				this->users[this->numberOfUsers].auth = SHA;
			}
			else if (auth == MD5)
			{
				password_to_key_md5((unsigned char*)authPass, (int)strlen(authPass), this->engineID, this->engineIDLength, (unsigned char*)this->users[this->numberOfUsers].authKey);
		...
		if (priv != 0)
		{
			if (privPass != NULL)
			{
				if (priv == DES)
				{
					if (auth == MD5)
					{
						password_to_key_md5((unsigned char*)privPass, (int)strlen(privPass), this->engineID, this->engineIDLength, (unsigned char*)this->users[this->numberOfUsers].privKey);
					...
					{
						password_to_key_sha((unsigned char*)privPass, (int)strlen(privPass), this->engineID, this->engineIDLength, (unsigned char*)this->users[this->numberOfUsers].privKey);
						this->users[this->numberOfUsers].priv = DES;
						Serial.print("privKey is: ");
						for (x = 0; x < 16; x++)
						{
...
void snmpAgent::fillOIDList()
{
	...
	this->addObject(usmStatsUnknownEngineIDs, 10, _COUNTER, &this->unknownEngineIDs);
	this->addObject(usmStatsUnknownUserNames, 10, _COUNTER, &this->unknownUserNames);
	this->addObject(usmStatsWrongDigests, 10, _COUNTER, &this->wrongDigests);
	this->addObject(usmStatsDecryptionErrors, 10, _COUNTER, &this->decryptionErrors);
	this->addObject(usmStatsUnsupportedSecLevels, 10, _COUNTER, &this->unsupportedSecLevels);
...
#else
	if (time(NULL) - this->prevMillis > 1)
	{
		// increment previous milliseconds
		this->prevMillis += 1;
...
int snmpAgent::v3userExists() // returns 1 if user is found
{
	...
	if (this->ver == 3)
	{
		...
		Serial.println(x);
		Serial.print((int)strlen(users[x].userName));
		Serial.print(" ");
...
bool snmpAgent::v2communityOK()
{
	...
	if (this->ver == 2)
	{
		...
		Serial.println(hede->_length);
		Serial.println((int)strlen(this->communityString));
		for (y = 0; y < 20; y++)
		{
...
void snmpAgent::sendTrapv3(char* receiverIP, unsigned char* oid, int len, const char* userName, authType auth, privType priv)
{
	...
	else
	{
		unsigned char trapTemplate[512] = {
			...
			0x01, 0x00, 0x04, 0x04, 0x6b, 0x65, 0x64, 0x69,  /* user name: kedi */
		...
		dummy->_length = (int)strlen(user->userName);
		...
		dummy = this->message.msgPrivParam;
		unsigned char salt[8] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08 };
		dummy->_content = salt;
		dummy->_length = 8;
